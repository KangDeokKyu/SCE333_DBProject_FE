...
/**
 * ChatRoomPage 컴포넌트는 다음 두 가지 화면을 하나로 처리합니다:
 *  1) 채팅방 목록(list) 화면
 *  2) 개별 채팅방(chat) 화면
 *
 * Props:
 *  - rooms: Array<{ id: number, title: string }>
 *      : 사용자가 참여 중인 채팅방 정보(게시글 ID와 제목을 담고 있음)
 *  - onBack: () => void
 *      : “목록”에서 상단의 뒤로가기 버튼을 누르면 App.jsx 로 돌아가도록 하는 콜백
 */
function ChatRoomPage({ rooms: initialRooms, onBack }) {
  // 1) roomsState: [{ id, name, messages: [...] }] 형태로 관리
  const [roomsState, setRoomsState] = useState(
    initialRooms.map((p) => ({
      id: p.id,
      name: p.title,
      messages: [], // 이 채팅방의 메시지 배열 (초기엔 빈 배열)
    }))
  )

  // 2) 현재 보고 있는 방 ID (null이면 목록 뷰, 숫자면 해당 채팅방 뷰)
  const [selectedRoomId, setSelectedRoomId] = useState(null)

  // 3) 사용자가 채팅 입력창에 입력 중인 메시지
  const [newMessage, setNewMessage] = useState('')

  // 4) 채팅창 하단으로 자동 스크롤하기 위한 ref
  const messagesEndRef = useRef(null)

  // 5) messagesState가 바뀔 때마다 자동 스크롤
  useEffect(() => {
  ...
  // ───────────────────────────────────────────────────────────────────────────────
  // ■ 목록(list) 화면에서 “방 나가기(leave)” 버튼을 눌렀을 때 호출되는 함수
  //    : 해당 방을 roomsState 배열에서 제거
  const handleLeaveRoom = (roomId) => {
    // 확인창 띄우고
    if (window.confirm('정말 이 채팅방에서 나가시겠습니까?')) {
      setRoomsState((prev) => prev.filter((r) => r.id !== roomId))
      // 만약 현재 상세 채팅 뷰로 들어간 방을 나갔다면 목록 뷰로 돌아감
            ...
            >
              ...
              <div
                style={{
                  ...
                  color: '#666',
                  margin:
                    msg.author === '나' ? '0 8px 0 0' : '0 0 0 8px',
                }}
              >
                {msg.author} ·{' '}
